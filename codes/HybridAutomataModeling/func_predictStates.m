...
function [predictionTrajectoryMatrix, predictionKFtrajectory, predGapFeatures, predCrossFeatures] = func_predictStates(kf, currentPedData, ~, currentTSActiveCarData, carTrackCurrentTimeStep, AVStates, pedTrackTimeStep, cw, annotatedImageEnhanced, resetStates, Prob_GapAcceptanceModel, Prob_CrossIntentModelCar, Prob_CrossIntentModelNoCar, Params, flag)
...
%                          ( strcmp(trackletData{trackletNo}.HybridState(end),'Approach') && strcmp(trackletData{trackletNo}.HybridState(end-1),'Crossing') && trackletData{trackletNo}.closestCW(end)~=trackletData{trackletNo}.closestCW(end-1)) )
                ...
                if ( size(trackletData{trackletNo}.trackLifetime,1) >= 2 &&...
                   ...
                   if (ped_CloseCar_id ~=0 && ped_CloseCar_id ~=inf && ~isempty(carTrackCurrentTimeStep))
                       ...
                       [CrossFeatures, flag] = func_compileCrossFeatures(currentPedData, trackletData, trackletNo, trackletStartNode, trackletEndNode, pedTrackTimeStep, timeStep, Params, flag, currentTSEgoCarData, carTrackCurrentTimeStep(egoActiveCarInd));
                   ...
                   % check crossIntent 
                   if flag.checkIntentWOEgo(trackletNo)
                       [CrossFeatures, flag] = func_compileCrossFeatures(currentPedData, trackletData, trackletNo, trackletStartNode, trackletEndNode, pedTrackTimeStep, timeStep, Params, flag);
                       mean_DTCurb = CrossFeatures.mean_DTCurb;
                       mean_ped_speed = CrossFeatures.mean_ped_speed;
                       mean_DTCW = CrossFeatures.mean_DTCW;
                       % predict
