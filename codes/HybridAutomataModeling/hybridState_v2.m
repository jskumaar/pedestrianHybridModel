...
%this function
function [tracksData] = hybridState_v2(tracksData, cw, flag, annotatedImage_enhanced, Params, trackletNo, resetStates)
%% 1) setup
% fixed parameters
...
%                      swInd = 0; % Not on any sidewalk 
       ...
       if  ( ( abs(ped_cw1_angle - pedHead) <= cwHeadingThreshold && dist_cw1 < cwDistThreshold ) || (dist_cw1 < cwCrossThreshold && onRoad))
            cwHeadingState(1) = 'Headed';
%             dist_cw_temp(1) = dist_cw1;
       elseif ( ( abs(ped_cw1_angle - pedHead) > cwHeadingThreshold + 10 || dist_cw1 > cwDistThreshold ) && (dist_cw1 > cwCrossThreshold) || ~onRoad )
            cwHeadingState(1) = 'Not_Headed';
       end
       if ( ( abs(ped_cw2_angle - pedHead) <= cwHeadingThreshold && dist_cw2 < cwDistThreshold ) || (dist_cw2 < cwCrossThreshold && onRoad))
            cwHeadingState(2) = 'Headed';
%             dist_cw_temp(2) = dist_cw2;
       elseif ( ( abs(ped_cw2_angle - pedHead) > cwHeadingThreshold + 10 || dist_cw2 > cwDistThreshold ) && (dist_cw2 > cwCrossThreshold) || ~onRoad )
            cwHeadingState(2) = 'Not_Headed';
       end
       if ( ( abs(ped_cw3_angle - pedHead) <= cwHeadingThreshold && dist_cw3 < cwDistThreshold ) || (dist_cw3 < cwCrossThreshold && onRoad))
            cwHeadingState(3) = 'Headed';
%             dist_cw_temp(3) = dist_cw3; 
       elseif ( ( abs(ped_cw3_angle - pedHead) > cwHeadingThreshold + 10 || dist_cw3 > cwDistThreshold ) && (dist_cw3 > cwCrossThreshold) || ~onRoad )
            cwHeadingState(3) = 'Not_Headed';
       end
       if ( ( abs(ped_cw4_angle - pedHead) <= cwHeadingThreshold && dist_cw4 < cwDistThreshold ) || (dist_cw4 < cwCrossThreshold && onRoad) )
            cwHeadingState(4) = 'Headed';
%             dist_cw_temp(4) = dist_cw4;
       elseif ( ( abs(ped_cw4_angle - pedHead) > cwHeadingThreshold + 10 || dist_cw4 > cwDistThreshold ) && (dist_cw4 > cwCrossThreshold) || ~onRoad )
       ...
       % update th cw distance (in m)
       distSW = dist_cw_temp(cwInd) * scaleDownFactor *orthopxToMeter;
    ...
    if ~flag.hybridStatePred(trackletNo)
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %% debug
        if (strcmp(walk_state, 'Stopping') && distSW > decZone)
            x=1;
        end
...
%             if ( strcmp(Region(end),"Sidewalk") && strcmp(cwHeadingCloseCW,'Headed') && (strcmp(walk_state, 'Walking') || (strcmp(walk_state, 'Stopping') && (distSW > decZone) ) ) )
            if ( strcmp(Region(end),"Sidewalk") && ( strcmp(walk_state, 'Walking') || (strcmp(walk_state, 'Stopping') && distSW > decZone) )...
...
%                annotatedImage_enhanced_w_tracks(-pixel_pos(2), pixel_pos(1)) = 60;
        ...
        % Note: Using absolute distance to center of crosswalk for now.
            elseif ( strcmp(Region(end),"Sidewalk") && strcmp(cwHeadingCloseCW,'Headed') && strcmp(walk_state, 'Stopping') && (distSW < decZone) )
               hybrid_state = 'Wait'; 
...
%             if ( strcmp(Region(end),"Sidewalk") && strcmp(cwHeadingCloseCW,'Not_Headed') && strcmp(walk_state, 'Walking') )
            elseif ( (strcmp(Region(end),"Sidewalk") || strcmp(Region(end),'')) && ( sign(dispRot_cw(cwInd,1))~= prevCWDispSign || strcmp(approachHeadingState,'Not_Headed') ) )
