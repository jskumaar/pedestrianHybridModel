...
for scene_id = 1:N_scenes
    ...
    for  ped_index = 1:length(ped_tracks)
        ...
        for scene_time = scene_start_time : skip_ts : scene_end_time
            ...
            % Step 2a: Find the active car and pedestrian tracks for this time step
            ...
            if ~isempty(activeCarTracks) 
                ...
                %Step 1a: Pedestrian data for an observaton window; this is done within the prediction loop as the ego-car can change within the prediction loop           
                if  loop_time > SampFreq % i.e. if there is data for over 1 s           
                    currentTSPedData{1} = currentPedData(loop_time-SampFreq + 1: loop_time, :);
                else
                    currentTSPedData{1} = currentPedData(1: loop_time, :);
                end
                ...
                if isempty(currentTSActiveCarData)
                 ...
                 if  iscell(currentTSActiveCarData.calcHeading(1))
                     x=1;
                 end
