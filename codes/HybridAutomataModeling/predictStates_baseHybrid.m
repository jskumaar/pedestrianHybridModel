...
function [predictionTrajectoryMatrix, predictionKFtrajectory, predGapFeatures] = predictStates_baseHybrid(kf, currentPedData, ~, currentTSActiveCarData, carTrackCurrentTimeStep, AVStates, pedTrackTimeStep, cw, annotatedImageEnhanced, reset, Prob_GapAcceptanceModel, Params, flag)
...
%                                 tempData2 = hybridState_v2(trackletData{trackletNo}, cw, flag, annotatedImageEnhanced, Params, length(trackletData{trackletNo}));
%                                 trackletData{trackletNo} = tempData2;
...
%                            flag.startCross(trackletNo) = false; % so that whether is reached is not checked again for the parent tracklet
                    ...
                    elseif (flag.reachCrosswalk(trackletNo) && ~flag.predHorizonEnd(trackletNo))
                            ...
                            if trackletData{trackletNo}.probCrossingIntent
                               [trackletData, trackletKfData, trackletProbability, trackletStartNode, trackletEndNode, trackletIsActive, trackletEventFlag, trackletGoal, newTrackletId, flag] = newTracklet(trackletData, trackletKfData, trackletProbability, trackletStartNode, trackletEndNode, trackletIsActive, trackletEventFlag, trackletGoal, node_no, trackletData{trackletNo}.probCrossingIntent, newTrackletId, newTrackletId, flag);
                               parentTracklet(newTrackletId) = trackletNo; 
